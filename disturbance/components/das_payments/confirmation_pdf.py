...
#        DPAW_HEADER_LOGO = os.path.join(settings.BASE_DIR, 'ledger', 'payments','static', 'payments', 'img','dbca_logo.jpg')
    ...
    doc.invoice = invoice
    doc.application_fee = application_fee
    owner = invoice.owner
...
def create_confirmation_pdf_bytes(filename, invoice, application_fee):
    confirmation_buffer = BytesIO()
    _create_confirmation(confirmation_buffer, invoice, application_fee)
